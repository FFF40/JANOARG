...
class PlayerHitboxVisualizer : MonoBehaviour
{
    ...
    public void PostRender(Camera cam)
    {
        ...
        for (int idx = 0; idx < DebugCoords.Count; idx++)
        {
            ...
            void updateVec()
            {
                vec = Common.main.MainCamera.ScreenToWorldPoint(new (Mathf.Cos(angle) * coord.Radius + coord.Position.x, Mathf.Sin(angle) * coord.Radius + coord.Position.y, 10));
            }
            updateVec();
