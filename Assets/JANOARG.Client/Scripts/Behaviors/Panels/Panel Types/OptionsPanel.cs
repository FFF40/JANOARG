...
namespace JANOARG.Client.Behaviors.Panels.Panel_Types
{
    public class OptionsPanel : MonoBehaviour
    {
        ...
        [Space] public TMP_Text SubtitleLabel;

        [Space] public ScrollRect ContentScroller;
        ...
        [HideInInspector] public List<OptionItem> CurrentItems;
        ...
        public void MakeTab(int tab)
        {
            ...
            Storage preferences = CommonSys.sMain.Preferences;
            ...
            switch (tab)
            {
                case 0:
                {
                    ...
                    GetOptionItemSample<StringOptionInput>()
                        .Limit = 16;

                    Spawn<StringOptionInput, string>(
                        "Player Name",
                ...
                case 1:
                {
                    ...
                    Spawn<VisualOffsetOptionInput, float>(
                        "Visual Offset",
                    ...
                    var msample = GetOptionItemSample<MultiFloatOptionInput>();
                    msample.Min = 0;
                    msample.Max = 100;
                    msample.Step = 5;
                    msample.Unit = "%";
                    ...
                    Spawn<MultiFloatOptionInput, float[]>(
                        ...
                        x =>
                        {
                            preferences.Set("PLYR:HitScale", x);
                            UpdatePlayerPreview();
                        }
                    );
                    ...
                    Spawn<FloatOptionInput, float>(
                        ...
                        x =>
                        {
                            preferences.Set("PLYR:FlickScale", x);
                            UpdatePlayerPreview();
                        }
                    );
                }
        ...
        public void InitFlickMeshes()
        {
            if (!freeFlickIndicator)
            {
                ...
                verts.AddRange(
                    new Vector3[]
                        { new(-1, 0), new(0, 2), new(0, -.5f), new(1, 0), new(0, -2), new(0, .5f) });
            ...
            if (!arrowFlickIndicator)
            {
                ...
                verts.AddRange(
                    new Vector3[]
                    {
                        new(-1, 0), new(0, 2.2f), new(1, 0), new(.71f, -.71f), new(0, -1), new(-.71f, -.71f)
                    });
        ...
        public TType Spawn<TType, TObject>(string title, Func<TObject> get, Action<TObject> set)
            where TType : OptionInput<TObject>
        {
            TType item = Instantiate(GetOptionItemSample<TType>(), OptionItemHolder);
            item.transform.localEulerAngles = Vector3.forward * 15;
            item.TitleLabel.text = title;
            ...
            yield return Ease.Animate(
                .5f, x =>
                {
                    float ease1 = Ease.Get(
                        Mathf.Pow(x, .5f), EaseFunction.Exponential,
        ...
        private static RectTransform RT(MonoBehaviour item)
        {
